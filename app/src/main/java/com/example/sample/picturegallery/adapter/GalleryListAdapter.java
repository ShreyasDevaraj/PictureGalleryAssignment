...
public class GalleryListAdapter extends RecyclerView.Adapter<GalleryListAdapter.PictureHolder> {

    private final List<GalleryItem> imageList;

    public GalleryListAdapter(final List<GalleryItem> list) {
        imageList = list;
    }
     ...
     */
    @Override
    public PictureHolder onCreateViewHolder(final ViewGroup parent, final int viewType) {
        final View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.gallery_item_row, parent, false);
    ...
    public void onBindViewHolder(final PictureHolder holder, final int position) {
        GalleryItem item = imageList.get(position);
        holder.label.setText(item.getCaption());
        holder.label.setTextColor(Color.WHITE);
        holder.imageView.setImageBitmap(item.getFaceImageId());
    }
    ...
    class PictureHolder extends RecyclerView.ViewHolder {
        ...
        PictureHolder(final View itemView) {
            super(itemView);
            imageView = (ImageView) itemView.findViewById(R.id.galleryItem);
            label = (TextView) itemView.findViewById(R.id.galleryItemName);
        }
    }
}
